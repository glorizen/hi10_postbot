...
export class PostbotComponent implements OnInit {
  ...
  public checksum: boolean = false;

  constructor(private ngZone: NgZone) {
    const response = `
    self.onmessage = function(e) {
      const str = e.data.str;
      const table = e.data.crcTable;
      const index = e.data.itemIndex;

      const stream = new Uint8Array(str);
      let crc = 0 ^ (-1);
      for (var i = 0; i < str.byteLength; i++) {
        crc = (crc >>> 8) ^ table[(crc ^ stream[i]) & 0xFF];
      }
      crc = (crc ^ (-1)) >>> 0;
      crc = (crc).toString(16).toUpperCase();
      crc = crc.length < 8 ? '0' + crc : crc;
    ...
    this.worker.onmessage = function (e) {
      _this_.files[e.data.index].crc32 = e.data.crc32;
    }
   }
      ...
      for (let index = 0; index < event.files.length; index++) {
        ...
        if (droppedFile.fileEntry.isFile) {
          ...
          fileEntry.file((file: File) => this.ngZone.run(() => {
            ...
            const humanSize = (file.size / 1024) > 1024 ?
              (
  ...
  public toggle(givenFile) {
    ...
    if (this.ignoredFiles.indexOf(givenFile) !== -1) {
      this.ignoredFiles.splice(this.ignoredFiles.indexOf(givenFile), 1)
    }

    givenFile.checked = !givenFile.checked;
    if (!givenFile.checked) {
      this.ignoredFiles.push(givenFile);
    }
  }
  ...
  public readFile(index) {

    const file = this.files[index].details;
    ...
    reader.onload = function(event: any) {
      _this_.worker.postMessage({
        str: event.target.result,
        crcTable: _this_.table,
        itemIndex: index
      });

      if (index + 1 < _this_.files.length) {
        _this_.readFile(index + 1);
      }
    }
